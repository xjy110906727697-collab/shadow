...
interface EntryListProps {
  ...
  onAddWordCard?: (entryId: string) => void;
}
...
export function EntryList({
  ...
  const adjustTime = (field: "startTime" | "endTime", delta: number) => {
    setEditForm((prev) =>
      prev ? { ...prev, [field]: Math.max(0, prev[field] + delta) } : null,
    );
  };
          ...
          {editing ? (
            <div className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  handleFieldChange("startTime", parseTimeInput(e.target.value))
                }
          ...
          {editing ? (
            <div className="flex items-center">
              <input
                ...
                onClick={(e) => e.stopPropagation()}
                onChange={(e) =>
                  handleFieldChange("endTime", parseTimeInput(e.target.value))
                }
                className="flex-1 px-2 py-1 border border-red-300 rounded-l text-sm font-mono"
              />
              <div className="flex border border-red-300 border-l-0 rounded-r divide-x divide-red-300">
          ...
          {editing ? (
            ...
            <div className="flex items-center justify-end gap-2 relative">
              ...
              <Popconfirm
                ...
                okText="是"
                cancelText="否"
                overlayClassName="[&_.ant-popconfirm-message]:text-sm"
              >
                <button
                  onClick={(e) => e.stopPropagation()}

  ...
  return (
    <div className="w-full px-4 md:px-6 py-4 pb-20 md:pb-4">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 dark:text-slate-100">学习方法</h1>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${
                        tagColors[article.tag] || 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400'
                      }`}>
                        {article.tag}
                      </span>
                    </div>
                    <h3 className={`text-sm font-semibold mb-1 ${isActive ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-slate-200'}`}>
                      {article.title}
                    </h3>
          ...
          <main className="flex-1 min-w-0">
            <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 md:p-8">
              <div className="flex items-center gap-3 mb-5">
                <h2 className="text-xl font-semibold dark:text-slate-100">{current.title}</h2>
                {current.date && (
                  <span className="text-xs text-gray-400 dark:text-slate-500 ml-auto">{current.date}</span>
                )}
              </div>

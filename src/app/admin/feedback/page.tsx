...
export default function AdminFeedbackPage() {
  ...
  const fetchFeedbacks = useCallback(async () => {
    ...
    try {
      ...
      setPagination(prev => ({ ...prev, total: data.pagination?.total || data.feedbacks.length }))
    } catch (error) {
      console.error('Failed to fetch feedbacks:', error)
    } finally {
      setLoading(false)
    }
  }, [pagination.current, pagination.pageSize, sortField, sortOrder, search, typeFilter])
  ...
  const columns: ColumnsType<Feedback> = [
    {
      ...
      render: (type: string) => {
        ...
        return (
          <Tag className={`px-2.5 py-1 rounded-full text-xs font-medium border-0 ${config.bg} ${config.color}`}>
            {type}
          </Tag>
        )
      },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-5">
        <Space wrap size="middle">
          ...
          <Select
            value={typeFilter}
            onChange={val => { setTypeFilter(val); setPagination(prev => ({ ...prev, current: 1 })) }}
            className="w-36"
            options={[
              { value: '', label: '全部类型' },
